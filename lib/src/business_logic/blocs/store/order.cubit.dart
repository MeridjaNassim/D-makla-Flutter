...
class OrderSelectedState extends OrderState {
  ...
  final int quantity;
  OrderSelectedState(this.menu, this.selectedVariant, this.selectedToppings,
      this.currentOrderPrice, this.quantity);
...
class OrderCubit extends Cubit<OrderState> {
  ...
  void setCurrentMenu(Menu menu,
      {Variant selectedVariant, ToppingList selectedToppings, int quantity}) {
  ...
  void setSelectedVariant(Variant variant) {
    ...
    if (state is OrderSelectedState) {
      Order order = Order(
          ...
          quantity: state.quantity);
      double price = order.getFullPrice();
      final newState = OrderSelectedState(
          state.menu, variant, state.selectedToppings, price, state.quantity);
      emit(newState);
    }
  }
  ...
  void incrementQuantity() {
    ...
    if (state is OrderSelectedState) {
      ...
      Order order = Order(
          ...
          quantity: quantity);
      double price = order.getFullPrice();
      final newState = OrderSelectedState(state.menu, state.selectedVariant,
          state.selectedToppings, price, quantity);
      emit(newState);
    }
  }
  ...
  void toggleTopping(Topping topping) {
    ...
    if (state is OrderSelectedState) {
      ...
      list.forEach((element) {
        ...
        if (element != topping)
          newToppings.addTopping(element);
        else
          shouldAdd = false;
      });
      ...
      Order order = Order(
          menu: state.menu,
          variant: state.selectedVariant,
          toppingList: newToppings,
          quantity: state.quantity);
      double price = order.getFullPrice();
      final newState = OrderSelectedState(state.menu, state.selectedVariant,
          newToppings, price, state.quantity);
