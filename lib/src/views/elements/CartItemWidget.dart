...
class _CartItemWidgetState extends State<CartItemWidget> {
  ...
  void _incrementOrder() {
    BlocProvider.of<CartBloc>(context).add(OrderQuantityInceremented(widget.order, 1));
  }
  void _decrement() {
    BlocProvider.of<CartBloc>(context).add(OrderQuantityDeceremented(widget.order, 1));
  }
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<CartBloc,CartState>(
      builder: (context,state)=>InkWell(
        ...
        onTap: () {
          Navigator.of(context).pushNamed('/Food', arguments: RouteArgument(id: widget.order.menu.id, heroTag: widget.heroTag));
        },
        child: Container(
